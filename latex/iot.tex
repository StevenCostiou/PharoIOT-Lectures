...
\frame{\frametitle{I2C Sensors}
\begin{block}{How does I2C work?}
\begin{itemize}
		\item Uses 2 wires SDA (serial data) and SCL (serial clock)
		\item Each device has an ID or a unique address
		\item 112 devices using 7-bit addressing
		\item 1008 devices using 10-bit addressing		
\end{itemize}
...
\begin{itemize}
		\item \texttt{PotBME280Device} class
\end{itemize}
...
\begin{itemize}
		\item \texttt{PotLCD1602Device} class to GPIO device
		\item \texttt{PotLCD1602DeviceI2C} class to I2C device
\end{itemize}
...
\begin{block}{Write an IOT application using an LCD and an BME280 sensor}
			...
			\begin{enumerate}
				\item Display the data of the sensor on the LCD each 1 second
			\end{enumerate}
\end{block}
}

\frame{\frametitle{Mini wheather station}
\begin{block}{Chapter 12 of Booklet}

			\begin{enumerate}
				\item Create an account on Thingspeak
				\item Create a class to display the sensor data on the LCD
				\item Create a class to send the data to the cloud
			\end{enumerate}
\end{block}
}

\frame{\frametitle{Web server}
\begin{block}{Chapter 13 of Booklet}

			\begin{enumerate}
				\item Create an html page and start the webserver
				\item Create methods to display the data on the web page
